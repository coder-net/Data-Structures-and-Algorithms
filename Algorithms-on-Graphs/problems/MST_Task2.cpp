...
double kruskal_MST(const vector<lpair>& vert, set<Node>& edges, ll k) {
    DisjointSet<ll> ds;
    for (ll i = 0; i < vert.size(); i++) {
        ds.make_set(i);
    }
    double weights = 0;
    while (ds.get_sets_count() > k) {
...
int main() {
    ...
    cout << std::setprecision(10) << kruskal_MST(vert, edges, k);
    return 0;
}
